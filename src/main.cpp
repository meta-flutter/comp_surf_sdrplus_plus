...
#include "../include/comp_surf_sdrpp/comp_surf_sdrpp.h"
#include "../third_party/SDRPlusPlus/core/src/context.h"
...
inline Context& getContext(comp_surf_Context* ctx) {
  checkContext(ctx);
  return *ctx->context;
}
}  // namespace
...
comp_surf_Context* comp_surf_initialize(const char* accessToken,
  ...
  ctx->context = std::make_unique<Context>(
      width, height, nativeWindow, miscPath, cachePath, assetsPath);
  return ctx;
}
